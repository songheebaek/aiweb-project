...
def _webshare_proxy_config() -> WebshareProxyConfig | None:
    username, password, locations = _webshare_settings()
...
# [DIAG] 배포/로컬 환경에서 프록시 Secret 로드 여부 확인 (값은 마스킹)
_ws_user, _ws_pass, _ws_locations = _webshare_settings()
print(
    "\n" + "=" * 50 + "\n"
    "[DIAG] 서버 시작 시점 환경 변수 점검:\n"
    f"  - Webshare username 로드 여부: {bool(_ws_user)} ({_mask(_ws_user)})\n"
    f"  - Webshare password 로드 여부: {bool(_ws_pass)}\n"
    f"  - Webshare 국가 필터: {_ws_locations or '전체 풀'}\n"
...
def _transcript_retry_settings() -> tuple[int, float]:
    ...
    try:
        attempts = int(os.getenv("TRANSCRIPT_FETCH_RETRIES", "3"))
    except ValueError:
        attempts = 3
    try:
...
def _proxy_selftest():
    ...
    for name, url in [
        ...
        try:
            ...
            print(f"[DIAG] selftest {name}: ERR {type(e).__name__}: {_scrub(e)[:90]}", flush=True)
    return True
...
def fetch_transcript(video_id: str) -> list:
    ...
    if webshare_proxy:
        ...
        try:
            print("[DIAG] Webshare 프록시로 자막 요청을 시작합니다.", flush=True)
            return _fetch_with_retries(_webshare_proxy_config, "Webshare")
        except Exception as e:
            print(f"[DIAG] Webshare 프록시 최종 실패: {type(e).__name__} - {_scrub(e)}", flush=True)
            raise
...
# [DIAG] 프록시 경유 접속 자가테스트 1회 (로그 확인용, 동작 영향 없음)
_proxy_selftest()
