...
def fetch_transcript(video_id: str) -> list:
    ...
    # 주거용 프록시는 요청마다 출구 IP가 회전 → 일부 IP는 유튜브가 막아 SSL EOF/타임아웃 발생(flaky).
    # 영상 자체 문제(자막 없음 등)는 즉시 올리고, 연결/프록시 실패는 새 IP로 재시도(대개 성공).
    def _fetch_with_retry(make_config, attempts=4):
        last_err = None
        for attempt in range(attempts):
            try:
                return _fetch(make_config())
            except (TranscriptsDisabled, NoTranscriptFound, VideoUnavailable):
                raise  # 영상 자체 문제 → 재시도 무의미
            except Exception as e:
                last_err = e
                print(f"[DIAG] transcript attempt {attempt + 1}/{attempts} failed: {type(e).__name__}", flush=True)
                if attempt < attempts - 1:
                    time.sleep(1)
        raise last_err

    # 1) Webshare residential (회전 IP라 실패 시 새 IP로 재시도)
    if ws_user and ws_pass:
        return _fetch_with_retry(
            lambda: WebshareProxyConfig(proxy_username=ws_user, proxy_password=ws_pass)
        )
