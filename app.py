...
def fetch_transcript(video_id: str) -> list:
    ...
    # 1) Webshare residential (실패 시 최대 3번 자동 재시도 로직 추가)
    if ws_user and ws_pass:
        last_err = None
        for attempt in range(3):
            try:
                print(f"[DIAG] Webshare 주거용 프록시 시도 {attempt + 1}회째...", flush=True)
                return _fetch(WebshareProxyConfig(proxy_username=ws_user, proxy_password=ws_pass))
            except (TranscriptsDisabled, NoTranscriptFound, VideoUnavailable):
                raise  # 영상 자체의 문제(자막 없음 등)는 재시도해도 의미 없으므로 즉시 에러 발생
            except Exception as e:
                print(f"[DIAG] Webshare 시도 {attempt + 1}회 실패: {type(e).__name__} - {e}", flush=True)
                last_err = e
                continue  # 에러가 나면 다음 루프로 넘어가서 다시 시도 (새 IP 할당 기대)
        
        # 3번 모두 실패한 경우 최종 에러를 throw하여 하위 로직(PROXY_URL 등)으로 넘어가거나 프론트에 알림
        print("[DIAG] Webshare 주거용 프록시 3회 시도 모두 실패", flush=True)
        if not proxy_urls:
            raise last_err if last_err else RuntimeError("Webshare 주거용 프록시 연결 실패")

    # 2) PROXY_URL 리스트 순회 (기존 코드 유지)
    if proxy_urls:
        last_err = None
        for purl in proxy_urls:
            try:
