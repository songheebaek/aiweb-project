...
EXAMPLE_URLS = [
    "https://www.youtube.com/watch?v=CDTEtw90G04",
    "https://www.youtube.com/watch?v=7p3w7fveSJk",
    "https://www.youtube.com/watch?v=wCYCYfNNGUM",
    "https://www.youtube.com/watch?v=2eqPBLgVH0U&t=192s",
    "https://www.youtube.com/watch?v=7OUWELKUac4&t=1202s",
]
...
def get_video_title(url: str) -> str | None:
    """YouTube oEmbed로 영상 제목 조회 (실패하면 None → URL로 폴백). 결과 캐시."""
    try:
        r = requests.get(
...
st.markdown(
    ...
    /* ----- 예시 영상 버튼 (요약 전 화면) ----- */
    .ex-head { font-size: 1.0rem; font-weight: 700; color: #1f2438; margin: 6px 2px 10px; }
    .st-key-examples .stButton > button {
        background: #ffffff; color: #2b3047; border: 1px solid #e6e8f2;
        font-weight: 600; justify-content: flex-start; text-align: left;
        box-shadow: 0 2px 8px rgba(40,50,90,.04);
    }
    .st-key-examples .stButton > button:hover {
        background: #f3f1fe; color: #5a4bd4; border-color: #c9c2f5;
    }
    .st-key-examples .stButton > button p { text-align: left; width: 100%; }
...
if st.session_state.summary:
    ...
    # 빈 화면 — 예시 영상(클릭하면 입력창에 자동 입력) + 3개 기능 소개 카드
    with st.container(key="examples"):
        st.markdown(
            '<div class="ex-head">👇 예시 영상으로 바로 체험해보세요 — 클릭하면 위 입력창에 자동 입력돼요 (직접 입력도 가능)</div>',
            unsafe_allow_html=True,
        )
        for i, ex_url in enumerate(EXAMPLE_URLS, 1):
            title = get_video_title(ex_url)
            label = f"{i}.  {title}" if title else f"{i}.  {ex_url}"
            st.button(
                label, key=f"ex{i}", on_click=use_example, args=(ex_url,),
                use_container_width=True,
            )
